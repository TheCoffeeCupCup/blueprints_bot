...
pub async fn establish_ftp_connection(
    ...
    logging::info!("Connecting to SFTP server \"{server_name}\"");

    let mut server = Server::establish_connection(&server_name, &server_creds)?;

    if check_files_amount {
        let files_amount = server.count_files_at_cwd()?;
...
async fn forward_file(
    ...
    for server in servers {
        ...
        upload_tasks.spawn(async move {
            ...
            server.upload_file(&file_info, &bytes, overwrite)
        });
    }
...
pub struct Server {
    ...
    _connection_guard: ssh2::Session,
    sftp: ssh2::Sftp,
    cwd: PathBuf,
}
...
impl Server {
    ...
    fn connect(
        ...
        let tcp = TcpStream::connect(&full_ip).map_err(|err| {
            logging::warning!("{connection_error} over TCP via `{full_ip}`: {err}");
            connection_error.clone()
        })?;

        let mut ssh_session = ssh2::Session::new().map_err(|err| {
            logging::warning!("Couldn't create SSH session for server \"{server_name}\": {err}");
            connection_error.clone()
        })?;

        ssh_session.set_tcp_stream(tcp);

        ssh_session.handshake().map_err(|err| {
            logging::warning!(
                "Couldn't perform a handshake on a SSH session for server \"{server_name}\": {err}"
            );
            connection_error.clone()
        })?;
        ...
        let sftp = ssh_session.sftp().map_err(|err| {
            logging::warning!("Couldn't initialize SFTP for server \"{server_name}\": {err}");
            connection_error.clone()
        })?;

        Ok(Self {
            server_name: server_name.to_string(),
            _connection_guard: ssh_session,
            sftp,
            cwd: PathBuf::new(),
        })
    }

    fn count_files_at_cwd(&mut self) -> Result<usize, String> {
        let server_name = &self.server_name;

        let files = self.sftp.readdir(&self.cwd).map_err(|err| {
    ...
    fn cwd(&mut self, folder_path: &str) -> Result<(), String> {
        let server_name = &self.server_name;
        let path = PathBuf::from(folder_path);

        self.sftp.stat(&path).map_err(|err| {
            logging::warning!(
                "Couldn't access blueprints folder `{folder_path}` on the server \"{server_name}\": {}",
                err
            );
    ...
    fn check_file_exists_in_cwd(&mut self, file_name: &str) -> bool {
        let file_path = self.cwd.join(file_name);
        self.sftp.stat(&file_path).is_ok()
    }
    ...
    fn upload_file(
        ...
        if !overwrite && self.check_file_exists_in_cwd(&file_info.filename) {
        ...
        let file_path = self.cwd.join(&file_info.filename);

        let mut file = self.sftp.create(&file_path).map_err(|err| {
            let error = format!("Couldn't create \"{file_name}\" on the server \"{server_name}\"");
            logging::error!("{error} (url: \"{file_url}\"): {err}");
            error
        })?;
        ...
        file.write_all(&bytes).map_err(|err| {
            let error =
                format!("Couldn't write to \"{file_name}\" on the server \"{server_name}\"");
            logging::error!("{error} (url: \"{file_url}\"): {err}");
            error
        })?;
